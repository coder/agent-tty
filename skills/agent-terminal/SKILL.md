...
If `agent-terminal` is not already available in the environment, fetch the current `README.md` from `coder/agent-terminal` on GitHub and follow its installation instructions before continuing. Otherwise use `agent-terminal` directly.
