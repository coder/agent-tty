...
export default async function simplifyWorkflow({
  args,
  phase,
  log,
  action,
  ...
  log('Captured simplify context', {
    ...
    diffCompactions: mux.utils.asArray(
      contexts.outputGitContext.diff?.workflowCompactions,
    ).length,
  });
  ...
  const reviewOutputs = mux
    ...
    .map(function (review) {
      return mux.utils.mustObject(
        review.structuredOutput,
        'review structured output is required',
      );
    });
...
function collectApplyPreflight(action, log, input, gitContext) {
  ...
  try {
    ...
    if (expectedBranch !== undefined)
      preflightInput.expectedBranch = expectedBranch;
...
function reviewedBranchForPreflight(reviewedStatus, input) {
  ...
  const branch =
    reviewedStatus && typeof reviewedStatus.branch === 'string'
      ? reviewedStatus.branch.trim()
      : '';
  ...
  try {
    ...
    if (!context || typeof context !== 'object') {
      return emptyGitContext(input, requestedRefs, [
        {
          name: 'reviewContext',
          error: 'git.reviewContext returned no context',
        },
      ]);
    }
    return {
    ...
    log(
      'Git review context action failed; continuing with empty simplify context',
      failure,
    );
...
function refsWithResolvedBase(refs, changedFiles) {
  const base =
    changedFiles &&
    typeof changedFiles === 'object' &&
    typeof changedFiles.base === 'string'
      ? changedFiles.base
      : '';
...
function isRequestedHeadCurrent(status, input) {
  ...
  const currentHeadSha =
    typeof status.headSha === 'string' ? status.headSha : '';
  ...
  if (
    !currentHeadSha ||
    !requestedHeadSha ||
    currentHeadSha !== requestedHeadSha
  )
    return false;
  ...
  if (requestedHeadRef)
    return Boolean(currentBranchRef && requestedHeadRef === currentBranchRef);
  if (
    currentBranch &&
    (requestedHead === currentBranch || requestedHead === currentBranchRef)
  ) {
    return true;
  }
...
function untrackedPaths(gitContext) {
  const paths = mux.utils
    ...
    .concat(
      mux.utils.asArray(
        gitContext.changedFiles && gitContext.changedFiles.untracked,
      ),
    )
    .map(filePath)
    .filter(Boolean);
...
function hasOnlyUntrackedChanges(gitContext) {
  return (
    ...
    !hasArrayItems(
      gitContext.changedFiles && gitContext.changedFiles.unstaged,
    ) &&
...
function compactDiffStat(diffStat) {
  return compactFields(
    diffStat,
    DIFF_FIELDS,
    mux.utils.compactText,
    DIFF_STAT_CHAR_BUDGET,
  );
}
  ...
  DIFF_FIELDS.forEach(function (field) {
    ...
    compacted[field] =
      included === value.length
        ? value
        : value.slice(0, included) + diffOmittedMessage(field);
    remaining -= included;
...
function diffSummary(diff, compactedDiff) {
  ...
  return {
    ...
    workflowCompactions: compactedDiff
      ? mux.utils.asArray(compactedDiff.workflowCompactions)
      : [],
...
function reviewPrompt(lane, input, reviewContext) {
  return [
    ...
    'You are the ' +
      lane.title +
      ' lane. Review every changed file in the supplied Git context.',
...
function parseArgs(args) {
  ...
  while (index < tokenized.tokens.length) {
    ...
    else if (token === '--review-only' || token === '--no-fix')
      input.fix = false;
    else if (token === '--fix') input.fix = true;
    else if (valueFlag && valueFlag.error)
      return { input: input, error: valueFlag.error };
    else if (valueFlag) {
      input[valueFlag.key] =
        valueFlag.key === 'maxFindings'
          ? mux.utils.boundedInt(
              valueFlag.value,
              DEFAULT_MAX_FINDINGS,
              1,
              Number.MAX_SAFE_INTEGER,
...
function tokenize(input) {
  ...
  for (let index = 0; index < input.length; index += 1) {
    ...
    if (escaped) {
      ...
      if (
        next === '\\' ||
        (next === quote && !isClosingQuote(input, index + 1))
      ) {
        escaped = true;
      } else current += char;
    } else if (quote) {
